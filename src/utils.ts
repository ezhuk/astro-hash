...
export async function readOrFetchFile(url: string, distDir: string): Promise<Buffer | null> {
  try {
    if (/^(https?:)?\/\//.test(url)) {
...
export function computeHash(data: Buffer | string, algorithm: 'sha256' | 'sha384'): string {
